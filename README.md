...
docker run -p 8000:8000 \
  -e MCP_SHARED_SECRET=your_secure_token \
  -v whatsapp-store:/app/whatsapp-bridge/store \
  whatsapp-mcp
```

The first time it runs, watch the container logs for the WhatsApp QR code and scan it with your phone (WhatsApp > Settings > Linked Devices):

```bash
docker logs -f <container-id>
```

The `store` volume persists your WhatsApp session and message history across restarts.

### Docker Compose

A `docker-compose.yml` is included. Create a `.env` file next to it with:

```
MCP_SHARED_SECRET=your_secure_token
```
